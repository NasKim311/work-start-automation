...
test.describe('백업본(.bak)이 유효한 경우', () => {
	test.use({
		...
		initialConfigBak: makeConfig([{ type: 'browser', title: '백업된사이트', value: 'https://example.com', delay: 0 }])
	});

	test('손상된 config.json은 .bak으로 자동 복구되고, 원본도 자가 치유된다', async ({ electronApp, userDataDir }) => {
		const { window, dialogs } = await catchFirstDialog(electronApp);
