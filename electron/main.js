...
function isValidProfile(p) {
	return p && typeof p === 'object' && typeof p.id === 'string' && typeof p.name === 'string' && Array.isArray(p.tasks) && p.tasks.every(isValidTask);
}
...
function normalizeAutoStartByDay(parsed) {
	...
	const fallbackId = typeof parsed.autoStartProfileId === 'string' ? parsed.autoStartProfileId : parsed.profiles[0].id;
...
function preserveCorruptedFile(raw) {
	const backupPath = path.join(path.dirname(configPath), `config.corrupted-${Date.now()}.json`);
	try {
...
const autoStartArgs = app.isPackaged ? ['--autostart'] : [app.getAppPath(), '--autostart'];
...
ipcMain.handle('import-config', async (event) => {
	...
	const trustChoice = dialog.showMessageBoxSync(win, {
		...
		detail: '가져온 설정에 포함된 프로그램 실행 항목은 별도 확인 없이 자동으로 실행될 수 있습니다. 출처를 모르는 파일은 가져오지 마세요.'
	});
	...
	try {
		...
		if (Array.isArray(parsed.profiles) && parsed.profiles.length > 0 && parsed.profiles.every(isValidProfile)) {
			return {
				profiles: parsed.profiles,
				activeProfileId: typeof parsed.activeProfileId === 'string' ? parsed.activeProfileId : parsed.profiles[0].id,
