...
function App() {
	...
	const updateActiveTasks = (updater: (tasks: Task[]) => Task[]) => {
		setProfiles((prev) => prev.map((p) => (p.id === activeProfileId ? { ...p, tasks: updater(p.tasks) } : p)));
	...
	useEffect(() => {
		...
		try {
			window.electronAPI.notifyDirtyState({ profiles, activeProfileId, autoStartByDay }, hasUnsavedChanges);
		} catch (error) {
	...
	useEffect(() => {
		const unsubscribe = window.electronAPI.onConfigLoadWarning(({ recovered, backupPath }) => {
			if (recovered) {
				alert('설정 파일이 손상되어 있었지만, 직전 백업본으로 자동 복구했습니다.\n지금 보이는 내용이 맞는지 확인해보세요.');
			} else {
				alert(
					'설정 파일이 손상되어 백업본으로도 복구하지 못해 기본 설정으로 시작합니다.\n' +
						(backupPath ? `손상된 원본은 다음 위치에 보존해두었습니다:\n${backupPath}` : '손상된 원본을 별도로 보존하는 데도 실패했습니다.')
				);
			}
		});
		return unsubscribe;
		...
		const init = async () => {
			try {
				...
				if (isAuto) {
					...
					const autoStartProfile = todaysProfileId ? config.profiles.find((p) => p.id === todaysProfileId) : undefined;
	...
	const deleteActiveProfile = () => {
		...
		if (!window.confirm(`"${activeProfile.name}" 세트를 삭제할까요? 안에 담긴 작업 ${activeProfile.tasks.length}개도 함께 삭제됩니다.`)) {
			return;
		}
		...
		<div className='min-h-screen font-sans pb-16'>
			...
			<div className='mn-hero pt-16 pb-24 px-6 flex flex-col items-center text-center'>
				...
				<p className='max-w-lg mx-auto leading-relaxed font-semibold' style={{ color: '#C9BFA0' }}>
					매일 아침 반복되는 업무 준비,
					<br />단 한 번의 클릭으로 내 책상에 완벽하게 세팅하세요.
				</p>
			</div>
			...
			<div className='max-w-4xl mx-auto px-6 -mt-14 relative z-20 space-y-6'>
				...
				<div className='mn-card p-4 sm:p-6'>
					...
					{isRenamingProfile ? (
						...
						<div className='flex flex-wrap items-center gap-2'>
							{profiles.map((p) => (
								<button key={p.id} onClick={() => setActiveProfileId(p.id)} className={`mn-type-tab ${p.id === activeProfileId ? 'active' : ''}`}>
									{p.name}
								</button>
							))}
					...
					{activeProfile && !isRenamingProfile && (
						<div className='flex flex-wrap items-center gap-4 mt-3 text-xs font-bold' style={{ color: '#A79C7F' }}>
							<button onClick={startRenameProfile} className='underline'>
								이름 변경
							</button>
							{profiles.length > 1 && (
					...
					<div className='grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3'>
						{WEEKDAY_ORDER.map((day) => (
							<div key={day} className='flex items-center gap-3'>
								<span className='text-sm font-bold w-4 text-center' style={{ color: '#5B5340' }}>
									{WEEKDAY_LABELS[day]}
								</span>
								<select
				...
				<div className='mn-card p-6 sm:p-8'>
					...
					<div className='flex flex-col sm:flex-row justify-between items-center gap-4'>
						<div className='flex items-center gap-3'>
							<span className='text-sm font-bold' style={{ color: '#5B5340' }}>
								윈도우 시작 시 자동 실행
							</span>
							<button onClick={toggleAutoStart} className={`mn-toggle-track ${autoStartEnabled ? 'on' : ''}`}>
								<span className='mn-toggle-dot' style={{ left: autoStartEnabled ? 22 : 3 }} />
							</button>
						</div>
						...
						<div className='flex items-center gap-3'>
							...
							>
								...
								{isRunning ? (runningTaskTitle ? `실행 중: ${runningTaskTitle}` : '실행 준비 중...') : '출근 시작하기'}
							</button>
							{isRunning && (
								<button onClick={handleStop} className='mn-stamp-secondary px-4 py-2.5 flex items-center gap-2' title='남은 작업 취소'>
				...
				<div className='mn-card p-6 sm:p-8'>
					<div className='flex items-center justify-between mb-2'>
						...
						<span className='text-xs font-bold' style={{ color: '#B9AC8C' }}>
							총 {tasks.length}개
						</span>
					</div>
					...
					{tasks.length === 0 ? (
						<div className='text-center py-10'>
							<p className='font-bold' style={{ color: '#8A8065' }}>
								아직 큐레이션된 작업이 없어요.
							</p>
							<p className='mt-1 text-sm' style={{ color: '#A79C7F' }}>
								위 폼에서 매일 사용하는 웹사이트나 프로그램을 추가해 보세요!
							</p>
						</div>
					) : (
						<TaskList
							...
							onUpdate={(i, updatedTask) => {
								updateActiveTasks((prev) => prev.map((t, idx) => (idx === i ? updatedTask : t)));
							}}
							onMove={(from, to) => {
