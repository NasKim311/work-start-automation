...
export default function TaskEntryForm({ initialValues, onSubmit, onCancel, submitLabel, variant = 'add' }: TaskEntryFormProps) {
	const [task, setTask] = useState<Task>(initialValues || { type: 'browser', title: '', value: '', delay: 1 });
	// delay는 항상 초 단위로 저장한다 — 분 단위는 입력 편의를 위한 표시 방식일 뿐,
	// 기존에 저장된 설정 및 main.js의 누적 딜레이 계산과 호환되도록 유지한다.
	const [delayUnit, setDelayUnit] = useState<DelayUnit>(task.delay > 0 && task.delay % 60 === 0 ? 'min' : 'sec');
			...
			<div className='flex flex-col gap-4'>
				...
				<div className='flex flex-col sm:flex-row gap-4 items-end sm:items-center'>
					<div className='flex-1 w-full relative'>
						...
						/>
						{task.type === 'program' && (
							<button type='button' onClick={selectFile} className='mn-icon-btn absolute right-0 top-1/2 -translate-y-1/2 p-1'>
					...
					<div className='flex items-center gap-3'>
						...
						<div className='flex gap-2 ml-2'>
							<button type='button' onClick={handleSubmit} disabled={!task.value.trim()} className='mn-stamp-primary px-4 py-2' title='저장'>
								{submitLabel}
							</button>
							{onCancel && (
								<button type='button' onClick={onCancel} className='mn-stamp-secondary px-4 py-2' title='취소'>
		...
		<form onSubmit={handleSubmit} className='flex flex-col gap-6'>
			...
			<div className='grid grid-cols-1 md:grid-cols-12 gap-5'>
				...
				<div className='md:col-span-2'>
					<button type='submit' disabled={!task.value.trim()} className='mn-stamp-primary w-full py-2.5'>
						{submitLabel}
					</button>
				</div>
			</div>
